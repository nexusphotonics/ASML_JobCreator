...
## Cell Structure:
MyJob.Cell.set_CellSize( [20.00, 27.2] )    # cell size [X,Y] in millimeters
MyJob.Cell.set_MatrixShift( [10, 0]) # shift by half a cell
...
#   1st WG layer
WG1 = MyJob.Image("H1","PBINTV2H1", sizeXY=[20, 30], shiftXY=[0,0])

## To expose on Layer 2 and more:

WG2 = MyJob.Image("H2", "PBINTV2H2", sizeXY=[20, 30], shiftXY=[0,0])
...
WG2.distribute( [-2,1], shiftXY = [0, -10.7607])
WG2.distribute( [0,1], shiftXY = [0, -10.7607])
WG2.distribute( [-1,1], shiftXY = [0.0099, -10.7607])
WG2.distribute( [1,1], shiftXY = [0.0099, -10.7607])

WG1.distribute( [-2,0], shiftXY = [0, -8.6393])
WG1.distribute( [0, 0], shiftXY = [0, -8.6393])
WG1.distribute( [-1,0], shiftXY = [0.0099, -8.6393])
WG1.distribute( [1,0], shiftXY = [0.0099, -8.6393])



OMET.distribute(cellCR= [-1,-1] , shiftXY = [-0.4401, -5.090])
OMET.distribute(cellCR= [0,-1] , shiftXY = [-0.4401, -5.090])
...
MyJob.export('CBR-LITH-W2T4_n.txt', overwrite=True) 
